...
object Visualization {
    def usable_name(intended_name: String): String = {
        intended_name.replace(".", "_")
    }

    object ControlFlowGraphVisualization {
        def apply(t: hssa.BindingTree.Relation): String = {

            def node_name(block: BindingTree.Block): String = usable_name(s"block${t.blocks.indexOf(block)}")
