...
case class TokenReader[T](input: SourceFile.SourceFileReader,
                          grammar: LexicalGrammarUtilities[T],
                         ) extends Reader[Token[T]] {
    private lazy val (parsedToken, r: SourceFile.SourceFileReader) = grammar(input)

    override lazy val first: Token[T] = {
        this.parsedToken.getOrElse(grammar.eof)
    }
    ...
    def readAll(): LazyList[Token[T]] = LazyList.iterate(this)(r => r.rest).map(r => r.parsedToken).takeWhile(t => t.isDefined).map(r => r.get)
}
